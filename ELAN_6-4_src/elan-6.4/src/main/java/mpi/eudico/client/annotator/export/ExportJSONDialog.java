...
public class ExportJSONDialog extends ClosableDialog implements ActionListener, ProgressListener
{
     ...
     * @param loadToServer boolean to determine if the dialog is for export to a file or to load to a server
     * @throws HeadlessException
     */
    public ExportJSONDialog (Frame owner, Transcription transcription, Selection selection, Boolean exportToServer) throws HeadlessException {
    	 super(owner, true);
	...
	public void exportToServerInThread() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
        		...
        		Map<String, Integer> exportedData = webAnnotationBuilder.exportAnnotationsToServer();
        		
        		if (exportedData.isEmpty()) {
        			progressInterrupted(this,"");
        		} else {
        			ExportedResultPane resultPane = new ExportedResultPane(exportedData);
        			JOptionPane.showMessageDialog(getParent(),resultPane ,ElanLocale.getString("ExportJSONToServerResult.Message.Title"), JOptionPane.INFORMATION_MESSAGE);
	...
	public void progressInterrupted(Object source, String message) {
		...
		JOptionPane.showMessageDialog(getParent(), ElanLocale.getString("ExportJSONToServerResult.Failure.Info"), ElanLocale.getString("ExportJSONToServerResult.Message.Title"), JOptionPane.INFORMATION_MESSAGE);
